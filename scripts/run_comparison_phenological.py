...
def extract_index_vals(subset_deks, index_df, cent, index_name, row):
    """Extract the index values from the specified files for the given plot centroid and index name, and append the results to the provided index DataFrame."""
    for dek in subset_deks:
        
        with rasterio.open(dek) as src:
            val = [x for x in src.sample([(cent.x,cent.y)])][0][0]
        index_df = pd.concat([index_df,pd.DataFrame({'id':[row.id],'dt':[dek.split('/')[-1][5:-4]],index_name:[val]})])
    return index_df
...
def extract_indices_year(spindices_year,rice_plots,VICI_folder,SWIA_folder, year):
    ...
    remove_ids = []
    id_month_dict = {}
    VICI_df = pd.DataFrame(columns = ['id','dt'])
    SWIA_df = pd.DataFrame(columns = ['id','dt'])
    ...
    for i,row in rice_plots.iterrows():
        ...
        season_months_id = [f'{i:02}' for i in range(int(season_months[0]),int(season_months[1])+1)]
        id_month_dict[row['id']] = season_months_id
            
        veg_state = [f'{i:02}' for i in range(int(season_months[0]),int(pi_month[state]))]
        mat_state = [f'{i:02}' for i in range(int(pi_month[state]),int(season_months[1])+1)]
            
        ### VEG STAGE
        veg_SWIA = flatten([sorted(glob.glob(os.path.join(SWIA_folder,f'SWIA-{year}-{month}*.tif'))) for month in veg_state])
        veg_VICI = flatten([sorted(glob.glob(os.path.join(VICI_folder,f'VICI-{year}-{month}*.tif'))) for month in veg_state])
    
    
        ## MAT STAGE
        mat_SWIA = flatten([sorted(glob.glob(os.path.join(SWIA_folder,f'SWIA-{year}-{month}*.tif'))) for month in mat_state])
        mat_VICI = flatten([sorted(glob.glob(os.path.join(VICI_folder,f'VICI-{year}-{month}*.tif'))) for month in mat_state])
    
        if i == 0:
            
            veg_VICI_df = extract_index_vals(veg_VICI, VICI_df, cent, 'VICI_veg', row)
            veg_SWIA_df = extract_index_vals(veg_SWIA, SWIA_df, cent, 'SWIA_veg', row)
            mat_VICI_df = extract_index_vals(mat_VICI, VICI_df, cent, 'VICI_mat', row)
            mat_SWIA_df = extract_index_vals(mat_SWIA, SWIA_df, cent, 'SWIA_mat', row)
            
        else:
            veg_VICI_df = extract_index_vals(veg_VICI, veg_VICI_df, cent, 'VICI_veg', row)
            veg_SWIA_df = extract_index_vals(veg_SWIA, veg_SWIA_df, cent, 'SWIA_veg', row)
            mat_VICI_df = extract_index_vals(mat_VICI, mat_VICI_df, cent, 'VICI_mat', row)
            mat_SWIA_df = extract_index_vals(mat_SWIA, mat_SWIA_df, cent, 'SWIA_mat', row)
            
        veg_df = veg_SWIA_df.merge(veg_VICI_df, on=['id', 'dt'])
        mat_df = mat_SWIA_df.merge(mat_VICI_df, on=['id', 'dt'])
        
    print(f'len veg_df : {len(veg_df.id.unique())}')
    print(f'len mat_df : {len(mat_df.id.unique())}')
    ...
    subset_veg = extract_stat_anom(veg_df)
    subset_mat = extract_stat_anom(mat_df)
        
    subset = subset_indices.merge(subset_veg, on=['id'])
    subset = subset.merge(subset_mat, on=['id'])
    
    return subset

def extract_stat_anom(spindices_year):
    """Extract statistical features (max, median, min, std, count) from the provided SPI/SPEI DataFrame for each plot ID, runs specifically for phenological variables."""
    ...
    subset_median = spindices_year.drop(columns='dt').groupby('id').mean()
    subset_median.rename(columns={col:col+'_mean' for col in subset_median.columns},inplace=True)
