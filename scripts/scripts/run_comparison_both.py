...
def extract_indices_year(spindices_year,rice_plots,VICI_folder,SM_folder):
    remove_ids = []
    id_month_dict = {}
    VICI_df = pd.DataFrame(columns = ['id','dt','SWIA'])
    ...
    for i,row in rice_plots.iterrows():
        ...
        subset_VICI = flatten([sorted(glob.glob(os.path.join(VICI_folder,f'SWIA-{year}-{month}*.tif'))) for month in season_months])
        
        for dek in subset_VICI:
            
            with rasterio.open(dek) as src:
                val = [x for x in src.sample([(cent.x,cent.y)])][0][0]
            VICI_df = pd.concat([VICI_df,pd.DataFrame({'id':[row.id],'dt':[dek.split('/')[-1][5:-4]],'SWIA':[val]})])
...
def extract_stat_subset(spindices_year):
    ...
    subset_countnz = subset_countnz.drop(columns=[col for col in subset_countnz.columns if 'SWIA' not in col])
    subset_countnz.rename(columns={'SWIA':'SWIA_count'},inplace=True)
...
out_mod_dir = r'/home/eoafrica/shared/MOFODRONI/Output/out_model_analysis_swia'
...
for zone in zones:
    ...
    for u_thresh in thresholds:
        ...
        VICI_folder = f'/home/eoafrica/shared/MOFODRONI/Output/analysis_swia/{zone:02d}zones/analysis_05_{u_thresh}/SWIA'
        ...
        #in_cols = [col for col in in_cols if 'SWIA' in col]
        ...
        #r2_score = np.sum(merged['SWIA_count'])
    ...
    zone_df.to_csv(os.path.join(out_mod_dir,'redo',f'{years}_{zone}_update.csv'),sep=';')
