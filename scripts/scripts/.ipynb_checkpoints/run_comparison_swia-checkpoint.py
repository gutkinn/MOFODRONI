...
for zone in zones:
    ...
    for u_thresh in thresholds:
        ...
        for year in years:
            ...
            print(f'year len: {len(spindices[spindices['dt'].str.contains(str(year))])}')
            subset = extract_indices_year(spindices[spindices['dt'].str.contains(str(year))],
                                                      rice_plots,
                                                      VICI_folder,
                                                      SWIA_folder)
            plots_state = rice_plots#[rice_plots['STATE']==state]
            
            merged_year = merge_plots_indices(plots_state,subset,year)
        ...
        plt.hist(merged['yield'],bins=20)
        plt.savefig(f'/home/eoafrica/shared/MOFODRONI/Output/images/test_hist_{year}.jpg')
        print(f'in_cols: {in_cols}')
        simple_reg = OLS(pred_col,merged[in_cols],missing='drop')
        r2_score = simple_reg.fit()
            #r2_score = np.sum(merged['VICI_count'])
        zone_dict[u_thresh]=[r2_score.rsquared_adj]
            #print(simple_reg.fit().rsquared,simple_reg.fit().f_pvalue)
        print(f'Model for {zone} zones and p{u_thresh} R²: {r2_score.rsquared_adj} RMSE: {np.sqrt(r2_score.mse_model)} pval: {r2_score.f_pvalue}')
    if zone == zones[0]:
        zone_df = pd.DataFrame(zone_dict)
    else:
        zone_df = pd.concat([zone_df,pd.DataFrame(zone_dict)])
    zone_df.to_csv(os.path.join(out_mod_dir,'test_quantiles',f'aR2_SWIA{year}_{zone}_update.csv'),sep=';')
