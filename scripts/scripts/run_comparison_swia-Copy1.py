...
def run_calc(year):
    ...
    for zone in zones:
        ...
        for u_thresh in thresholds:
            ...
            in_cols = [col for col in in_cols if 'SPI' not in col]
            in_cols = [col for col in in_cols if 'SPEI' not in col]
        ...
        r2_df.to_csv(os.path.join(out_mod_dir,'with_zeros',f'R2_SWIANDVIA{year}_{zone}_update.csv'),sep=';')
        mse_df.to_csv(os.path.join(out_mod_dir,'with_zeros',f'MSE_SWIANDVIA{year}_{zone}_update.csv'),sep=';')
        p_df.to_csv(os.path.join(out_mod_dir,'with_zeros',f'p_SWIANDVIA{year}_{zone}_update.csv'),sep=';')
